...
export async function POST(req: NextRequest) {
  ...
  let body: { company_name: string; email: string; industry: string; address?: string; logo_url?: string; color1?: string; color2?: string; company_phone?: string; quote: PartialQuote; utm_source?: string; utm_medium?: string };
  try {
    body = await req.json();
  } catch {
  ...
  const { company_name, email, industry, address, logo_url, color1, color2, company_phone, quote, utm_source, utm_medium } = body;
  ...
  if (company_phone) payload.company_phone = company_phone;
